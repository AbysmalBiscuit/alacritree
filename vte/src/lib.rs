...
impl<const OSC_RAW_BUF_SIZE: usize> Parser<OSC_RAW_BUF_SIZE> {
    ...
    ///
    /// Printable characters go out in runs rather than one call each, because
    /// a performer writing into a grid can set up a row once and fill it.
    #[inline]
    fn ground_dispatch<P: Perform>(performer: &mut P, text: &str) {
        ...
        for (offset, c) in text.char_indices() {
            ...
            if run < offset {
                performer.print_str(&text[run..offset]);
            }
            performer.execute(c as u8);
            run = offset + c.len_utf8();
        }

        if run < text.len() {
            performer.print_str(&text[run..]);
        }
    }
}
...
pub trait Perform {
    ...
    ///
    /// `text` never contains a control character, so an implementation able to
    /// write a whole run at once should override this and skip the per-`char`
    /// setup the default does.
    #[inline]
...
mod tests {
    ...
    /// Printable characters reach the performer as whole runs, broken only
    /// where a control character interrupts them.
    #[test]
    fn ground_dispatch_batches_printable_runs() {
        ...
        impl Perform for Recorder {
            fn print_str(&mut self, text: &str) {
                self.runs.push(Run::Print(text.into()));
            }

            fn execute(&mut self, byte: u8) {
                self.runs.push(Run::Execute(byte));
            }
        }
        ...
        // A C1 control and a printable multibyte character both sit mid-input,
        // so the split has to run on characters rather than on bytes.
        parser.advance(&mut recorder, "ab\ncd\u{85}éf".as_bytes());
