...
pub trait Handler {
    ...
    /// A run of printable characters to be displayed.
    ///
    /// `text` holds no control characters, so a handler writing into a grid can
    /// resolve the cursor's row once for the run instead of once per character.
    #[inline]
    ...
    T: Timeout,
{
    ...
    fn print_str(&mut self, text: &str) {
        ...
        // REP repeats whatever printed last, which after a run is its tail.
