...
fn parse_search_depth(raw: Option<&str>) -> SearchDepth {
    match raw {
        None => SearchDepth::default(),
        Some("workspaces") => SearchDepth::Workspaces,
        Some("sessions") => SearchDepth::Sessions,
        Some(other) => {
...
struct RawUi {
    ...
    /// once `show_panes` is on, an agentless pane.  Default false: an
    /// existing config keeps the toggle's original session-only behavior.
    ...
    /// How far a sidebar query reaches: "workspaces" (default) matches project
    /// and worktree names only; "sessions" also matches session titles and
    /// herdr agent names.
...
impl RawConfig {
    fn into_config(self) -> Config {
        ...
        let ui = UiTheme {
            ...
            search_depth: parse_search_depth(self.ui.search_depth.as_deref()),
