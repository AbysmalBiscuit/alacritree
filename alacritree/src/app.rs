...
use crate::config::{
    ...
    DEFAULT_UPSTREAM_UNTRACKED_ICON, DEFAULT_WORKTREE_ICON, DEFAULT_WORKTREE_MAIN_ICON, IconStyle,
    Icons, LastSessionClose, PathStyleConfig, ScrollbarStyle, SearchDepth, SearchScope,
    SidebarTooltips, TextEmphasis, profile_command,
};
    ...
    fn sidebar_snapshot(&mut self, skip_worktree: Option<&Path>) -> sidebar_focus::TreeSnapshot {
        ...
        let inputs = sidebar_focus::ObservedInputs::capture(
            ...
            sidebar_focus::UiInputs {
                ...
                toggles_apply: self.sidebar.filter.toggles_apply(self.search_scope),
                pr_generation: pr_generation_for(
                    self.pr_cache.generation(),
                    any_pr_toggle_active(&self.sidebar.filter, self.search_scope),
                ),
                active_workspace,
                active_branch,
