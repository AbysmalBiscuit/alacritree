...
impl AlacritreeApp {
    pub(super) fn dispatch_action(
        ...
        match action {
            ...
            BindingAction::Unsupported(name) => self.dispatch_unsupported_action(&name),
        }
    }
    ...
    fn dispatch_named_action(&mut self, ctx: &Context, action: NamedAction, origin: ActionOrigin) {
        match action {
            ...
            NamedAction::SelectPreviousSession => {
            ...
            NamedAction::SelectNextWorkspace => {
                self.cycle_workspaces(ctx, 1);
            },
            NamedAction::SelectPreviousWorkspace => {
                self.cycle_workspaces(ctx, -1);
            },
    ...
    fn dispatch_unsupported_action(&self, name: &str) {
