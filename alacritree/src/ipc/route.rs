...
pub(crate) enum DeferredRequest {
    ...
    AttachMultiplexerPane { side: String, terminal_id: String, no_focus: bool },
    /// Waits until the created pane's session can be read.
    CreateMultiplexerPane { side: Option<String>, workspace: Option<PathBuf>, no_focus: bool },
}
...
impl From<IpcRequest> for Route {
    fn from(request: IpcRequest) -> Self {
        ...
        match request {
            ...
            Req::AttachMultiplexerPane { side, terminal_id, no_focus } => {
                Self::App(Deferred::AttachMultiplexerPane { side, terminal_id, no_focus }.into())
            },
            Req::CreateMultiplexerPane { side, workspace, no_focus } => {
                Self::App(Deferred::CreateMultiplexerPane { side, workspace, no_focus }.into())
            },
