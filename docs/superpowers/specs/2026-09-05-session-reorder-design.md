...
**`origin` stays in the range even when those filters would drop it.** A session whose project was removed is `Parent::Detached` (`sidebar_focus.rs:24`) and appears in no `workspace_order`; a session in a worktree being deleted is on a filtered row. Neither may be barred from moving *within* its own workspace, and neither has anywhere else to go, so the rule is one sentence: the range is the filtered list, with `origin` restored at its own position if missing. Under every scope a detached session therefore gets a range of one.
...
**The cursor follows the session,** staying the same `SidebarRow::Session(id)` across a move within a workspace, so a held key walks the session down the list rather than the two parting company on the first press.
